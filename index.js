...
module.exports = function (content, file, conf) {
    ...
    fragment.childNodes.forEach(function (node) {
        ...
        if ((type === 'script' || type === 'template') && output[type].length > 0) {
            throw new Error('[fis3-parser-vuecomponent] Only one <script> or <template> tag is ' + 'allowed inside a Vue component.');
        }
        else {
            output[type].push({
                content: content,
    ...
    if (output['script'].length) {
        scriptStr = 'define("' + file.filename + '", function(require){\n' + output['script'][0].content;
    ...
    if (output['template'].length) {
        ...
        scriptStr += '\nexports.template = _vueTemplateString;\n';
        scriptStr += '\nreturn exports;\n});';
    }
    else {
        scriptStr += '\nexports.template = "";\n';
